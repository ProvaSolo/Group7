...
// <<< VERSIONE FINALE E CORRETTA DELLA FUNZIONE DI FLUSH >>>
static void lpspi_flush_txfifo(NXPS32K358LPSPIState *s)
{
    // Controlla PRIMA se un trasferimento è possibile.
    // Se non c'è una parola intera da inviare O non c'è spazio per riceverla,
    // non iniziare nemmeno la transazione sul bus (non toccare il CS).
    if ((fifo8_num_used(&s->tx_fifo) < 4) || (fifo8_num_free(&s->rx_fifo) < 4)) {
        ...
        // Aggiorna comunque lo stato, potrebbe essere necessario aggiornare i flag.
        lpspi_update_irq(s);
        return;
    }
    ...
    // Un trasferimento è possibile, quindi inizia la transazione sul bus.
    DB_PRINT("Asserting CS%d for transfer burst.\n", pcs);
    qemu_set_irq(s->cs_lines[pcs], 0);

    // Esegui un ciclo e trasferisci tutti i dati possibili in una sola volta.
    ...
    // La transazione sul bus è finita.
    DB_PRINT("De-asserting CS%d after transfer burst.\n", pcs);
    qemu_set_irq(s->cs_lines[pcs], 1);

    // Il "messaggio" complessivo è finito? Lo è solo se la TX FIFO è vuota.
...
static void nxps32k358_lpspi_write(void *opaque, hwaddr addr, uint64_t val64, unsigned int size)
{
    ...
    case S32K_LPSPI_CFGR1: s->lpspi_cfgr1 = value; break;
    case S32K_LPSPI_CCR: s->lpspi_ccr = value; break;
